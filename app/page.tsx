...
.ek-hero-badge { display: inline-flex; align-items: center; gap: 12px; margin-bottom: 24px; }
.ek-hero-badge-line { height: 1px; width: 32px; background: #60A5FA; }
.ek-hero-badge-text { font-size: 11px; font-weight: 500; letter-spacing: 0.3em; text-transform: uppercase; color: #60A5FA; }
...
.ek-badges { display: flex; flex-wrap: wrap; justify-content: center; gap: 16px; margin-top: 40px; }
...
.ek-label { font-size: 11px; font-weight: 500; letter-spacing: 0.4em; text-transform: uppercase; color: #60A5FA; margin-bottom: 12px; }
.ek-section-h2 { font-size: clamp(24px, 3.5vw, 36px); font-weight: 700; margin-bottom: 6px; }
.ek-section-h3 { font-size: clamp(20px, 2.8vw, 30px); font-weight: 600; margin-bottom: 20px; }
.ek-divider { width: 48px; height: 2px; border-radius: 2px; background: linear-gradient(90deg, #3B82F6, #60A5FA); margin-bottom: 28px; }
...
.ek-stat-card { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 16px; text-align: center; }
...
.ek-feature-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.09); border-radius: 12px; padding: 18px; display: flex; align-items: flex-start; gap: 12px; transition: all 0.25s; }
...
.ek-condition-card { background: #fff; border: 1px solid #DBEAFE; border-radius: 16px; padding: 24px; transition: all 0.25s; }
.ek-condition-card:hover { border-color: #3B82F6; box-shadow: 0 8px 30px rgba(59,130,246,0.12); }
.ek-condition-icon-wrap { width: 40px; height: 40px; border-radius: 10px; background: #EFF6FF; display: flex; align-items: center; justify-content: center; font-size: 18px; color: #1E62C8; margin-bottom: 10px; flex-shrink: 0; }
...
.ek-step-num { width: 48px; height: 48px; border-radius: 50%; border: 1px solid rgba(96,165,250,0.45); display: flex; align-items: center; justify-content: center; margin: 0 auto 12px; font-size: 14px; font-weight: 600; color: #60A5FA; }
...
.ek-principle-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.10); border-radius: 16px; padding: 28px; }
...
.ek-indication-badge { display: inline-block; font-size: 9px; font-weight: 500; letter-spacing: 0.1em; border: 1px solid rgba(96,165,250,0.4); color: #60A5FA; padding: 2px 6px; border-radius: 4px; margin-bottom: 6px; }
.ek-golden-box { background: rgba(59,130,246,0.12); border: 1px solid rgba(96,165,250,0.25); border-radius: 12px; padding: 20px; display: flex; align-items: flex-start; gap: 14px; }
.ek-golden-icon { width: 36px; height: 36px; border-radius: 50%; background: rgba(96,165,250,0.2); display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 16px; }
...
.ek-hearing-card { background: #fff; border: 1px solid #DBEAFE; border-radius: 16px; padding: 28px; transition: all 0.25s; }
.ek-hearing-card:hover { border-color: #3B82F6; transform: translateY(-4px); box-shadow: 0 12px 40px rgba(59,130,246,0.14); }
.ek-hearing-icon { width: 48px; height: 48px; border-radius: 12px; background: #0B1F45; color: #60A5FA; display: flex; align-items: center; justify-content: center; font-size: 20px; margin-bottom: 20px; }
...
.ek-subsidy-bar { background: linear-gradient(135deg, #0B1F45, #1249A0); border-radius: 16px; padding: 28px; display: flex; flex-direction: column; gap: 20px; align-items: flex-start; }
...
.ek-photo-placeholder { border-radius: 16px; overflow: hidden; aspect-ratio: 4/5; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); display: flex; flex-direction: column; align-items: center; justify-content: center; margin-bottom: 20px; }
.ek-doctor-card { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 24px; }
...
.ek-cred-card { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.09); border-radius: 12px; padding: 20px; }
...
.ek-philosophy-card { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 20px; text-align: center; transition: border-color 0.25s; }
...
/* 푸터 */
.ek-footer-cta { padding: 56px 0; display: flex; flex-direction: column; gap: 32px; border-bottom: 1px solid rgba(255,255,255,0.08); }
...
.ek-top-line { position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, #60A5FA 40%, #3B82F6 60%, transparent); }
`

/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   DATA
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
const PHONE = '042-000-0000'   
const KAKAO = 'https://open.kakao.com/'  
...
const HEARING_SERVICES = [
  { icon: '🔊', title: '서울고려 난청 클리닉', desc: '순음청력검사(PTA)·어음청력검사·임피던스검사를 포함한 정밀 청력 평가로 원인별 맞춤 치료를 제공합니다.', points: ['정밀 청력검사 일체', '노인성·소음성·감각신경성 난청', '소아 청력검사'] },
  { icon: '🎵', title: '서울고려 이명 클리닉', desc: '이명도검사·최소차폐레벨 검사로 이명의 특성을 정밀 분석하고 이명재훈련치료(TRT) 등을 복합 적용합니다.', points: ['이명 전문가 자격증 보유', '이명재훈련치료(TRT)', '고압산소치료 병행'] },
  { icon: '🎧', title: '서울고려 보청기 전문', desc: '보청기전문가자격증을 보유한 전문의가 청력도에 맞춘 보청기를 처방하고 정밀 피팅합니다.', points: ['보청기전문가 자격증 보유', '실이측정(REM) 기반 피팅', '국가 보조금 처방전 발급'] },
]
...
function Header() {
  ...
  return (
    <header className="ek-header" style={{
      ...
      <AnimatePresence>
        {open && (
          <motion.div className="ek-mobile-menu" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ background: 'rgba(11,31,69,0.98)' }}>
  ...
  return (
    <section className="ek-hero">
      ...
      <div className="ek-wrap ek-hero-inner">
        ...
        <motion.div className="ek-badges" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.2 }}>
          {['보건복지부 인증 이비인후과 전문의','수면 전문의','어지럼증 전문가 자격증','이명 전문가 자격증','보청기전문가 자격증'].map(b => (
            <span key={b} className="ek-badge-item"><span style={{ color: '#60A5FA' }}>·</span> {b}</span>
          ))}
        </motion.div>
      </div>

      <motion.div className="ek-scroll-indicator" animate={{ y: [0, 9, 0] }} transition={{ repeat: Infinity, duration: 2.4, ease: 'easeInOut' }}>
...
function SleepSection() {
  ...
  return (
    <section id="sleep" ref={ref} className="ek-section" style={{ background: 'linear-gradient(150deg, #0B1F45 0%, #0D2757 60%, #133580 100%)' }}>
          ...
          <motion.div variants={fadeLeft()} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
            ...
            <div className="ek-stats-grid">
              {[{ v: '15+', l: '수면다원\n검사 항목' }, { v: '98%', l: '진단 정확도' }, { v: '수면', l: '전문의\n자격 보유' }].map(s => (
                <div key={s.v} className="ek-stat-card"><div className="ek-stat-val">{s.v}</div><div className="ek-stat-lbl">{s.l}</div></div>
              ))}
            </div>
    ...
    <section id="dizziness" ref={ref} className="ek-section" style={{ background: '#fff' }}>
      <div className="ek-wrap">
        ...
        <div className="ek-condition-grid">
          {CONDITIONS.map((c, i) => (
            <motion.div key={c.name} className="ek-condition-card" variants={fadeUp(0.08 + i * 0.07)} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                <div className="ek-condition-icon-wrap">{c.icon}</div><div className="ek-condition-name">{c.name}</div>
              </div>
              <div className="ek-condition-desc">{c.desc}</div>
            </motion.div>
          ))}
        </div>
        <motion.div className="ek-step-box" variants={fadeUp(0.3)} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
          <div className="ek-step-box-title">대전 어지럼증 <span style={{ color: '#60A5FA' }}>정밀 검사 과정</span></div>
          <div className="ek-step-grid">
            {STEPS.map(s => (
              <div key={s.step} className="ek-step-item"><div className="ek-step-num">{s.step}</div><div className="ek-step-title">{s.title}</div><div className="ek-step-desc">{s.desc}</div></div>
            ))}
          </div>
        </motion.div>
      </div>
...
function HBOTSection() {
  const { ref, inView } = useReveal()
  return (
    <section id="hbot" ref={ref} className="ek-section" style={{ background: 'linear-gradient(160deg, #050E1F 0%, #0B1F45 50%, #0D2757 100%)' }}>
      <div className="ek-top-line" />
      ...
      <div className="ek-wrap" style={{ position: 'relative', zIndex: 1 }}>
        <motion.div variants={fadeUp()} initial="hidden" animate={inView ? 'visible' : 'hidden'} style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <h3 className="ek-section-h3" style={{ color: '#60A5FA' }}>최신 고압산소챔버 가동</h3>
          <div className="ek-divider ek-divider-center" />
        </motion.div>
        <div className="ek-hbot-layout">
          <motion.div className="ek-principle-card" variants={fadeUp(0.1)} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
            <div className="ek-principle-icon">💠</div><div className="ek-principle-title">치료 원리</div>
            {HBOT_PRINCIPLES.map((t, i) => (
              <div key={i} className="ek-principle-item"><span className="ek-principle-num">{'①②③'[i]}</span><p>{t}</p></div>
            ))}
          </motion.div>
          <motion.div variants={fadeUp(0.18)} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
            ...
            <div className="ek-indication-grid">
              {HBOT_INDICATIONS.map((item, i) => (
                <div key={item} className="ek-indication-item" style={{ background: i < 2 ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.05)', border: i < 2 ? '1px solid rgba(96,165,250,0.3)' : '1px solid rgba(255,255,255,0.08)' }}>
                  {i < 2 && <div className="ek-indication-badge">대표</div>}<p>{item}</p>
                </div>
              ))}
            </div>
...
function HearingSection() {
  const { ref, inView } = useReveal()
  return (
    <section id="hearing" ref={ref} className="ek-section" style={{ background: '#EFF6FF' }}>
      <div className="ek-wrap">
        <motion.div variants={fadeUp()} initial="hidden" animate={inView ? 'visible' : 'hidden'} style={{ textAlign: 'center', marginBottom: 56 }}>
        ...
        <div className="ek-hearing-grid">
          {HEARING_SERVICES.map((s, i) => (
            <motion.div key={s.title} className="ek-hearing-card" variants={fadeUp(0.1 + i * 0.1)} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
              <div className="ek-hearing-icon">{s.icon}</div><div className="ek-hearing-title">{s.title}</div><div className="ek-hearing-desc">{s.desc}</div>
              <ul className="ek-hearing-points">{s.points.map(p => <li key={p} className="ek-hearing-point"><div className="ek-hearing-point-dot" />{p}</li>)}</ul>
            </motion.div>
          ))}
        </div>
        <motion.div className="ek-subsidy-bar" variants={fadeUp(0.35)} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
          <div>
            ...
            <div className="ek-subsidy-desc">처방전 발급부터 보청기 선택, 피팅까지 정밀하게 해결하세요.</div>
          </div>
...
function DoctorSection() {
  const { ref, inView } = useReveal()
  return (
    <section id="doctor" ref={ref} className="ek-section" style={{ background: 'linear-gradient(150deg, #0B1F45 0%, #0D2757 100%)' }}>
      <div className="ek-top-line" />
      <div className="ek-wrap">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} style={{ textAlign: 'center', marginBottom: 56 }}>
        ...
        <div className="ek-doctor-layout">
          <motion.div initial={{ opacity: 0, x: -28 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }}>
            <div className="ek-photo-placeholder" style={{ padding: 0, overflow: 'hidden' }}>
              <img src="/doctor.png" alt="원장 사진" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <div className="ek-doctor-card">
              <div className="ek-doctor-role">원장</div><div className="ek-doctor-name">박성빈 원장</div>
              <div className="ek-doctor-spec">이비인후과 전문의 · 수면 전문의</div><div className="ek-doctor-divider" />
              <div className="ek-doctor-quote">"환자의 수면의 질·삶의 질이 근본적으로 개선될 때까지 함께하는 의료를 추구합니다."</div>
            </div>
          </motion.div>
          <div>
            <div className="ek-cred-list">
              {CREDENTIALS.map((cred, i) => (
                <motion.div key={cred.cat} className="ek-cred-card" initial={{ opacity: 0, x: 28 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.15 + i * 0.08 }}>
                  <div className="ek-cred-header"><div className="ek-cred-dot" /><div className="ek-cred-cat">{cred.cat}</div></div>
                  <ul className="ek-cred-items">{cred.items.map(it => <li key={it} className="ek-cred-item"><span className="ek-cred-dash">–</span>{it}</li>)}</ul>
                </motion.div>
              ))}
            </div>
            <motion.div className="ek-philosophy-grid" initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.5 }}>
              {PHILOSOPHY.map(p => (
                <div key={p.title} className="ek-philosophy-card"><div className="ek-philosophy-icon">{p.icon}</div><div className="ek-philosophy-title">{p.title}</div><div className="ek-philosophy-desc">{p.desc}</div></div>
              ))}
            </motion.div>
          </div>
        </div>
  ...
  return (
    visible && (
      <div className="ek-floating">
        <a href={KAKAO} target="_blank" rel="noopener noreferrer" className="ek-float-btn" style={{ background: '#FEE500', fontSize: 22 }}>💬</a>
        <a href={`tel:${PHONE}`} className="ek-float-btn ek-pulse" style={{ background: '#3B82F6' }}>📞</a>
      </div>
    )
  )
}
